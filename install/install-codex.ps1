# Codex flavor installer (Windows orchestrator)
#
#   Get-ChildItem -Recurse | Unblock-File
#   powershell -ExecutionPolicy Bypass -File install\install-codex.ps1
#
# Resumable via %USERPROFILE%\.harness-bundle-codex-state.json
# Shared flywheel/WSL stages already completed by another flavor are skipped.
# ntm already has --cod. This installer does not invent an ntm agent type.
...
$script:StateFile = Join-Path $env:USERPROFILE '.harness-bundle-codex-state.json'
...
if ($Update) {
    Write-Host "-> Update mode: Codex config/skills will be re-deployed; flywheel installs stay untouched" -ForegroundColor Cyan
...
Write-Host " Codex harness installer" -ForegroundColor Green
Write-Host " Same flywheel, Codex as the agent CLI. ntm --cod already exists." -ForegroundColor DarkGray
Write-Host "==============================================" -ForegroundColor Green
Write-Host ""
...
if (-not (Test-StageDone $state 'preflight')) {
    ...
    if ($freeGB -lt 8) { Write-Warn2 "Only ${freeGB}GB free on C: - Codex config will still deploy; flywheel/WSL wants ~15GB." }
    if (-not (Get-CodexExe)) {
        Write-Err2 "codex not found. Install the official Codex CLI first, then re-run."
        Write-Host "  Official: npm install -g @openai/codex"
        Write-Host "  Docs: https://learn.chatgpt.com/codex/cli"
        exit 1
    }
    Write-Ok "preflight passed (codex present, disk ${freeGB}GB free)"
...
# =================== Stage 3: Codex login ===================
if (-not (Test-StageDone $state 'codex-login')) {
    Write-Info "Stage 3: Codex login"
    $auth = Join-Path $env:USERPROFILE '.codex\auth.json'
    $codex = Get-CodexExe
    if (-not (Test-Path $auth)) {
        Write-Host ""
        Write-Host "MANUAL STEP: open a NEW terminal, run:  codex login" -ForegroundColor Yellow
        Write-Host "Complete ChatGPT / device auth, then come back and press Enter." -ForegroundColor Yellow
        Read-Host "Press Enter once login is complete"
        if (-not (Test-Path $auth)) {
            Write-Warn2 "No $auth yet. If you signed in inside the TUI, continue; otherwise re-run after codex login."
        }
    }
    Write-Ok "Codex CLI present ($codex)"
    Complete-Stage $state 'codex-login'
}

# =================== Stage 4: Config + skills deploy ===================
if (-not (Test-StageDone $state 'config-deploy')) {
    Write-Info "Stage 4: deploying Codex skills + config"
    $codexDir = Join-Path $env:USERPROFILE '.codex'
    $agentsDir = Join-Path $env:USERPROFILE '.agents'
    New-Item -ItemType Directory -Force @(
        (Join-Path $codexDir 'skills'),
        (Join-Path $codexDir 'rules'),
    ...
    Write-Info "copying skills payload to ~/.codex/skills and ~/.agents/skills"
    Copy-Item (Join-Path $script:BundleRoot 'skills\*') (Join-Path $codexDir 'skills') -Recurse -Force
    Copy-Item (Join-Path $script:BundleRoot 'skills\*') (Join-Path $agentsDir 'skills') -Recurse -Force

    Convert-Template (Join-Path $script:BundleRoot 'config\codex\AGENTS.md.template') (Join-Path $codexDir 'AGENTS.md')
    $dstAgentsMd = Join-Path $env:USERPROFILE 'AGENTS.md'
    if (-not (Test-Path $dstAgentsMd)) {
        Convert-Template (Join-Path $script:BundleRoot 'config\codex\AGENTS.md.template') $dstAgentsMd
    } else {
    ...
    Get-ChildItem (Join-Path $script:BundleRoot 'config\rules') -Filter '*.md' | ForEach-Object {
        Convert-Template $_.FullName (Join-Path $codexDir "rules\$($_.Name)")
        Convert-Template $_.FullName (Join-Path $agentsDir "rules\$($_.Name)")
    }
    Get-ChildItem (Join-Path $script:BundleRoot 'config\codex\rules') -Filter '*.md' | ForEach-Object {
        Convert-Template $_.FullName (Join-Path $codexDir "rules\$($_.Name)")
        Convert-Template $_.FullName (Join-Path $agentsDir "rules\$($_.Name)")
    }

    Copy-Item (Join-Path $script:BundleRoot 'config\codex\hooks\trauma_guard.py') (Join-Path $codexDir 'hooks\trauma_guard.py') -Force
    Copy-Item (Join-Path $script:BundleRoot 'config\codex\hooks\post-compact-reminder.py') (Join-Path $codexDir 'hooks\post-compact-reminder.py') -Force
    Convert-Template (Join-Path $script:BundleRoot 'config\codex\hooks.json') (Join-Path $codexDir 'hooks.json')
...
"@
    if (Test-Path $cfg) {
        ...
        if ($existing -notmatch '\[features\]') {
            Add-Content -Path $cfg -Value $hooksBlock
            Write-Ok "appended [features] hooks = true to config.toml"
        } elseif ($existing -notmatch 'hooks\s*=') {
            Add-Content -Path $cfg -Value "hooks = true`n"
            Write-Ok "appended hooks = true under existing [features]"
        } else {
            Write-Ok "config.toml already has features.hooks"
        }
    } else {
        Set-Content -Path $cfg -Value $hooksBlock.TrimStart() -Encoding utf8
        Write-Ok "wrote new config.toml with features.hooks = true"
    }

    Write-Warn2 "Inside Codex, run /hooks and trust the harness hooks (Codex skips untrusted user hooks)."
...
if ($SkipWSL) {
    ...
    if (-not $wslReady) {
        ...
        Write-Host "  Then re-run this installer. Codex config already deployed; this stage resumes."
    } else {
...
Write-Info "Stage 7: Codex MCP registrations"
powershell -ExecutionPolicy Bypass -File (Join-Path $script:BundleRoot 'install\mcp-register-codex.ps1')
...
if (-not (Test-StageDone $state 'smoke')) {
    Write-Info "Stage 11: Codex smoke test"
    $code = Invoke-GitBash -ScriptPath (Join-Path $script:BundleRoot 'install\smoke-test-codex.sh')
    if ($code -eq 0) { Write-Ok "smoke test green"; Complete-Stage $state 'smoke' }
    else { Write-Warn2 "smoke test reported failures (exit $code). Fix and re-run install-codex.ps1 -Update" }
}

Write-Host ""
Write-Host "==============================================" -ForegroundColor Green
Write-Host " Codex install complete" -ForegroundColor Green
Write-Host "==============================================" -ForegroundColor Green
Write-Host " Completed stages: $($state.completed -join ', ')"
Write-Host ""
Write-Host " Next steps:"
Write-Host "   1. Open a NEW terminal"
Write-Host "   2. codex doctor"
Write-Host "   3. Inside Codex: /hooks  (trust the harness hooks)"
Write-Host "   4. Read docs/field-guide/01-philosophy.md then 07-codex-flavor.md"
Write-Host "   5. Swarm panes: ntm spawn <project> --cod=N  (already in official ntm)"
Write-Host ""
