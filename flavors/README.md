...
| **OpenCode CLI** | Native: skills from `~/.claude/skills`, rules from `~/.claude/CLAUDE.md` plus `instructions` glob on `~/.claude/rules`. | Thin adapter: `install\install-opencode.ps1` |

You can run all five on one machine. You edit rules and skills under `~/.claude/` once.
