...
Split from the global CLAUDE.md 2026-07-30 (content preserved verbatim; Agent Mail/networking sections updated to the verified current architecture).
...
**Default tool for any shell call with variables, captured content, loops, or chaining: Python `subprocess.run([cmd, *args], cwd=..., env=...)`** — Python's argv-list form passes args via the OS exec interface, zero shell layers, zero parsing. When you would otherwise build a `wsl ... bash -lc '...'` invocation that takes input from a variable, captured output, or a file body — write a 30-line Python wrapper instead. (Worked first-try for a bead-creation wrapper after a `bash -lc` attempt re-evaluated backticks.)
...
```bash
wsl -d Ubuntu -u root -- bash -l <<'BASHEOF'
cd /mnt/c/Users/USER/Documents/AgentVaults/example-swarm
...
- "I just need to capture-pane on 6 panes really quick" — exactly the case that burned a full session of orchestration time April 2026 because `for i in 0 1 2 3 4 5; do tmux capture-pane -t SESSION:0.$i -p; done` ran 6 times against the SAME pane (loop var didn't expand). Heredoc.
...
- Most flywheel tools (cass, cm, br, bv, caam, dcg, ubs, slb) are installed on BOTH sides — `/root/.local/bin/<tool>` (WSL) and `C:\Users\USER\.local\bin\<tool>.exe` or scoop shims (Windows). Same commands work on both sides. (ntm and ubs are WSL-only.)
...
| --------------------------------- | --------------------------------------------- | -------------------------------------------------------------------- |
| `/root/.claude/.credentials.json` | `/mnt/c/Users/USER/.claude/.credentials.json` | OAuth tokens (one OAuth = both OSes authed)                          |
| `/root/.claude/projects`          | `/mnt/c/Users/USER/.claude/projects`          | Claude Code session JSONL files (Windows cass picks up WSL sessions) |
| `/root/.claude/skills`            | `/mnt/c/Users/USER/.claude/skills`            | One skills directory read by both sides                              |
| `/root/.local/share/caam`         | `/mnt/c/Users/USER/.local/share/caam`         | caam vault — profiles visible from both sides                        |

### Boot hook (the actual WSL daemon launcher — no systemd)

`/etc/wsl.conf` sets `mountFsTab = false` and `[boot] command = /usr/local/sbin/mount-fast-data.sh`. On every WSL **cold boot** (not per-shell, not per-login) that script: waits up to 30s for `/mnt/c`, runs `mount -a` (fast-data bind mounts), idempotently starts the credentials-symlink watcher, and idempotently starts the Agent Mail server (`am serve-http --host 0.0.0.0 --port 8765`). Log: `/root/.local/share/mount-fast-data.log`. `mountFsTab=false` exists because WSL processes `/etc/fstab` before `/mnt/c` is mounted — fstab bind-mounts targeting `/mnt/c` would fail at native boot. A vestigial disabled systemd unit for agent-mail exists; the boot hook is the real mechanism.
...
**Concrete failure mode this prevents:** a 6-pane swarm hammering `br update` / `br close` against `/mnt/c/.../.beads/beads.db` produces 30-second SQLite write-lock timeouts; same swarm against `/root/projects-data/<proj>/.beads/beads.db` resolves writes in <100ms. 9p's POSIX-lock emulation queues every concurrent writer behind the prior fsync still draining through WSL→NTFS.
...
```python
import json
WIN, WSL = "/mnt/c/Users/USER/.claude.json", "/root/.claude.json"
...
A wrapper that does this automatically (`caam-safe`) is in design — see `ObsidianVault/caam-safe-wrapper/`.

### Mid-session caam corruption avoidance

Documented in caam README FAQ: switching profiles while Claude Code is running corrupts both the old AND new account's session. Best practice: kill all Claude windows → `caam activate <profile>` → relaunch. The caam-safe wrapper (when built) automates this entire flow including identity sync.
...
`C:\Users\USER\Documents\caam-stack-smoke-test.sh` — end-to-end checks across this whole architecture. Run after any infrastructure change to verify nothing regressed. **Known-stale as of 2026-07-30:** phases 5/10/11 still assert the retired Windows-native Agent Mail daemon and will false-fail; a rewrite for the WSL-native architecture (with `127.0.0.1` probes) is planned. Until then, triage failures against the current-architecture notes in `References/tooling-update-runbook.md` before believing them.
