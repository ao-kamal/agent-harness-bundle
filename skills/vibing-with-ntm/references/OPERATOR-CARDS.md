...
ntm --robot-tail=<session> --panes=cod --lines=5 | grep -iE 'model|error'
```
...
ntm --robot-send=<session> --panes=cc --msg="Fresh commit <short-sha>. Review it — is the Result type the right variant? Is the error context preserved? Any regressions? Apply depth-gate (OC-034) before declaring clean."
```
