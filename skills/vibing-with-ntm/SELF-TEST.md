...
Validate that the skill's description reliably triggers on realistic operator phrasings. For each phrase below, the skill should be chosen over sibling skills (`/ntm`, `/agent-mail`, `/br`, `/bv`, `/caam`).
...
| "br ready is returning empty but I know work exists" | `/br` or `/fixing-beads-problems` |
| "compute betweenness centrality on my bead graph" | `/bv` |
| "add a new Claude Max account to CAAM" | `/caam` |
| "which Gemini model does the review swarm use" | `/code-review-gemini-swarm-with-ntm` |
| "provision a fresh Ubuntu box for agents" | `/provision-new-machine-generic` |
