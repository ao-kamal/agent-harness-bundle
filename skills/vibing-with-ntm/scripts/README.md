...
Zero-token helpers (executed, never loaded into Claude's context) for repetitive operator-loop operations. All scripts print and exit; none mutate state without explicit user confirmation. Per AGENTS.md RULE 1: none of these scripts ever delete files.
...
- **None of these scripts modify tmux, beads, git, or filesystem state.** They only read and report.
...
(Convergence-check exits 0 on converge, which `/loop` can honor as a stop condition.)
