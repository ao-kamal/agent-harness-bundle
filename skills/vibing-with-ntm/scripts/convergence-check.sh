...
#
# Usage: convergence-check.sh <session> <repo_path> [prev_inflight_count]
...
STATE="/tmp/vibing-convergence-$SESSION.state"
...
while IFS= read -r line; do
  ...
  if echo "$line" | grep -qiE "$CONV_REGEX"; then
    CONV_PANES=$((CONV_PANES + 1))
  fi
done < <(ntm --robot-tail="$SESSION" --lines=30 2>/dev/null | jq -r '(.panes // {}) | (if type=="object" then [.[]] else . end) | .[]? | ((.lines // .tail // []) | if type=="array" then join(" ") else (.|tostring) end) | gsub("\n";" ")' 2>/dev/null || true)
...
C3=0; [ "$READY" -eq 0 ] && [ "$INFLIGHT" -eq "$PREV_INFLIGHT" ] && C3=1

cat <<EOF
convergence-check for session=$SESSION repo=$REPO
  C1 (commits_1h=0)           = $C1  (actual: $COMMITS)
  C2 (all panes converged)    = $C2  ($CONV_PANES / $TOTAL_PANES panes match)
  C3 (ready=0, inflight flat) = $C3  (ready=$READY, inflight=$INFLIGHT, prev=$PREV_INFLIGHT)
