...
**NEVER overwrite an existing progress file.** Previous skill loop runs
contain valuable audit data. Use a DATED filename per run:

`.skill-loop-progress-YYYY-MM-DD-[skill-name].md`

Example: `.skill-loop-progress-2026-04-08-structural-audit.md`

If `.skill-loop-progress.md` already exists with content from a previous
run, leave it alone. Create your new dated file alongside it.
...
1. Find the active progress file in the project root — look for
   `.skill-loop-progress-*.md` files sorted by date, or the base
   `.skill-loop-progress.md` if no dated files exist
