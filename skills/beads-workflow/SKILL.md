...
```bash
# 1. Initialize beads in project (if not already done)
br init

# 2. Convert plan to beads (see THE EXACT PROMPT below)
#    ALWAYS label beads with a project name (see Labels section)
...
```
OK so now read ALL of [YOUR_PLAN_FILE].md; please take ALL of that and elaborate on it and use it to create a comprehensive and granular set of beads for all this with tasks, subtasks, and dependency structure overlaid, with detailed comments so that the whole thing is totally self-contained and self-documenting (including relevant background, reasoning/justification, considerations, etc.-- anything we'd want our "future self" to know about the goals and intentions and thought process and how it serves the over-arching goals of the project.). The beads should be so detailed that we never need to consult back to the original markdown plan document. IMPORTANT: Before creating any beads, run `br label list-all`. If beads from other projects already exist, ask the user for a label and apply `-l <label>` to every bead you create. Remember to ONLY use the `br` tool to create and modify the beads and add the dependencies. Use ultrathink.
```
...
br create "Title" -t feature -p 1 -l project-name    # Create bead WITH LABEL
br update <id> --status in_progress
br close <id> --reason "Done"
br reopen <id>                       # If needed
```

### Labels (Project Separation)

Multiple projects share the same `.beads/` database. Labels prevent
beads from different projects mixing together.

**ALWAYS label every bead at creation time with `-l project-name`.**

```bash
# Create with label
br create "Some task" -t task -p 2 -l agnosticism

# Add label to existing bead
br label add <id> project-name

# Filter by project
br list --label agnosticism          # Only agnosticism beads
br ready --label agnosticism         # Actionable in this project only
bv --robot-triage-by-label           # Triage grouped by project

# See all labels and counts
br label list-all
```

Without labels, `br list` and `bv --robot-triage` show ALL beads from
ALL projects — making it impossible to focus on one stream of work.

**When to label:**
Before creating beads, run `br label list-all`.
- If beads already exist in the database (from another project), you MUST
  label yours to prevent mixing. Ask the user for a label name.
- If the database is empty (fresh init or single project), labels are
  optional — no collision risk.
- NEVER guess or invent a label silently — label collisions (e.g.,
  `agnosticism` vs `agnostic`) fragment the project view
- When delegating to subagents and labels are in use, ALWAYS include the
  label in the prompt so the subagent applies it to every bead it creates
