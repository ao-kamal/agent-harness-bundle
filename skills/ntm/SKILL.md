...
14. **WSL-spawned Claude Code panes loop on a hook-approval dialog.** Panes hit a recurring "settings.json to update hooks / Do you want to proceed?" gate that LOOPS (approving re-triggers it; only `Escape` exits, ending the turn), blocking all work — even under `--dangerously-skip-permissions` (it is a separate hook-approval gate). Cause: hooks in the WSL `~/.claude/settings.json` (e.g. a `dcg` PreToolUse guard + PCR) require per-session approval that does NOT persist in auto-spawned panes, so it re-prompts every turn. Fix: BEFORE spawning, back up + empty the WSL hooks (`d['hooks']={}`), then restore after the swarm. Also strip any leaked Windows-only env such as `CLAUDE_CODE_USE_POWERSHELL_TOOL` (`pwsh` is absent in WSL, so it forces a broken tool path). Diagnosed 2026-08 — ~6 respawns to isolate.
15. **`ntm --robot-*` commands HANG under heavy pane CPU load.** With several Opus-class panes thinking hard, `--robot-send` / `--robot-health-oauth` / `--robot-is-working` can take minutes or stall the tick. For operator ticks under load, prefer fast local `tmux` / `br` / `git`, wrap slow calls in `timeout 40`, and expect them to background (>120s) — read the output file rather than re-running.
16. **`ntm spawn` dying with "spawn recovery canceled: context deadline exceeded"** is the cm-memory recovery/context injection timing out (they are different code paths). Pass BOTH `--no-cass-context` AND `--no-recovery` on the spawn to skip them.
