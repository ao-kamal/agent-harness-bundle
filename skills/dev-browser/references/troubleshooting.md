...
| Piped stdin (`$script \| dev-browser` in PowerShell) exits 0 with **zero output**, or cold start dies with `Exception setting "ShowWindow": property cannot be found` | PowerShell pipe swallows the daemon handshake and all stdout/stderr; the CLI's internal `Win32_ProcessStartup` ShowWindow set fails under PS 5.1 on cold start | Write the script to a `.js` file and run `dev-browser --browser <name> run script.js`. `saveScreenshot(buf, name)` always lands in `~/.dev-browser/tmp/<name>` regardless of invocation style |
